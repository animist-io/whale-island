...
+ **Contracts API:** Each node maintains its own account and has its own contract deployed to the blockchain. By calling this contract's event methods and implementing a pre-defined API in their own contract code, Dapps can have a node independently verify their client's presence at a location in time. They can also ask the node to broadcast a signal on an arbitrary port in order to coordinate the behavior of several mobile clients in the same location. For more on how to implement these behaviors in your own contracts see [animist-io/wallowa](https://github.com/animist-io/wallowa).
