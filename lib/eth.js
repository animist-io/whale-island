...
exports.getPresenceReceipt = function (pin, signedPin) {
  ...
  if (client){
    console.log('Testing null sha3 null input: --> ' + util.sha3(null))
    const time = Date.now()
    let timeHash = util.addHexPrefix(util.sha3(time).toString('hex'))
    let addressHash = util.addHexPrefix(util.sha3(client).toString('hex'))
    let signedTime = web3.eth.sign(config.animistAccount, timeHash)
    let signedAddress = web3.eth.sign(config.animistAccount, addressHash)
