 ...
 * @param  {Function} cb (Optional) Callback to execute when an event is logged and saved in the DB
 */
const startBroadcastRequestsFilter = exports.startBroadcastRequestsFilter = function( contractAddress, startBlock, cb ){

    // Instantiate Events contract
    let eventsContract = web3.eth.contract(config.eventsABI);
    eventsContract = eventsContract.at(contractAddress);

    eventsContract.LogBroadcastRequest(null, {fromBlock: startBlock, toBlock: 'latest'}, (err, event) => {
...
class AnimistBroadcaster {
    ...
    start() {
        ...
        startBroadcastRequestsFilter(config.eventsContractAddress, web3.eth.blockNumber);
    }    
};
