...
function generateBeaconSignature(uuid, major, minor){
    ...
    //console.log('msgHash len: ' + msgHash.length);
    ...
    getLastSavedBlock().then( last => {
        ...
        messageFilter.watch((err, event) => {
            ...
            if (err) { // Publish errors
                ...
                server.addPublication(event.args);
                cb();
    ...
    beaconFilter.watch(( err, event ) => {
        ...
        if (err)
            ...
            if (instance){
                ...
                //------------------------------------------------------------
                console.log();
                console.log('Debugging intermittent failure on Travis here');
...
exports.fromRpcPummel = function(uuid){
    ...
    //while( i < 50 ){
        ...
        //let major = generateRandom2ByteInt();
        //let minor = generateRandom2ByteInt();
        let sig = generateBeaconSignature(uuid, major, minor);
        console.log('uuid: ' + uuid + ' major: ' + major + ' minor: ' + minor)
        console.log(JSON.stringify(sig) + '\n');
        //if(sig.v === undefined) { console.log('\n\nJACKPOT\n\n'); return } 

        minor = 22205;
        sig = generateBeaconSignature(uuid, major, minor);
        console.log('GOOD: uuid: ' + uuid + ' major: ' + major + ' minor: ' + minor)
        console.log(JSON.stringify(sig) + '\n');
