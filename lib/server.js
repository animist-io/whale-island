...
/** 
 Gets pin as string or eth-lightwallet object. (Some requests pass just the signed pin, 
 some embed the pin in an object with other data.)
...
/** 
 Checks that signed tx object from is valid - i.e. that it was signed by the same sender
 as the pin, that the signature verifies.
...
const parseSignedTx = function(data, client){
    ...
    try {     
        ...
        // Case: bad client address - e.g. pin signing failed (it will be an error code)
        if (typeof client != 'string')
            return {ok: false, val: codes.INVALID_PIN }

        // Case: Not object, not object w/tx, tx not string
        ...
        // Case: Cannot extract address or address is not client
        else if ( util.addHexPrefix(parsed.tx.getSenderAddress()) != client )
            return { ok: false, val: codes.INVALID_TX_SENDER_ADDRESS };

        // Case: Tx signature and sender address don't match
        else if ( !parsed.tx.verifySignature() )
            return { ok: false, val: codes.INVALID_TX_SIGNATURE };

        // Case: ok
        else
            return { ok: true, val: parsed.tx}

    // Case: Unknown parse failure.
    } catch( err ){ 
        return { ok: false, val: codes.INVALID_JSON_IN_REQUEST };
    }
};
...
const onAuthAndSubmitTx = function(data, offset, response, callback ){
    ...
    if (parsedPin.ok && parsedTx.ok){
        ...
        eth.authTx( [pin, lastPin], parsedPin.val )
            .then( txHash => {
                eth.submitTxWhenAuthed(txHash, parsedTx.val, parsedPin.val );
                out = new Buffer(JSON.stringify(txHash));
                setTimeout( () => self.updateValueCallback(out), 50);   
            })
            .catch( err => { 
