...
const TX_HASH_LENGTH = 0x42; // Fear of numbers


// ----------------------------------- Locals ------------------------------------------

/**
 @var killGetContract {bool} Flag to continue/finish getContract tranmission 
 @var sendQueue {Array} Array of type Buffer repr the contract to send
 @var pin {String} Published random string, changes ~30 sec
 @var lastPin {String} Prev pin, kept in case a req overlaps change.
*/
let killGetContract;      
let sendQueue = [];       
let pin = rs.generate();  
...
/** 
 Gets pin as string or eth-lightwallet object. 
 (Some endpoint reqs pass just the pin, some embed the pin in an object with other data.)
 @method extractPinFromJSON
 @param {String} JSON formatted string or object 
 @return {String || Object }. Returns null on error. 
*/
...
/**
 Validates format of signedPin (A check done before attempt to extract address from it).
 @method parseSignedPin
 @param {String} JSON convertible a signed string or an object with v,r,s fields.
 @returns {Object} ok: boolean status, val: signed pin OR hex error code.
*/
...
/** 
 ...
 @param {String} JSON formatted object
 @param {String} Hex prefixed address extracted from pin signing
 @return {Object} ok: boolean status, val: tx string or error code
*/
...
/**
 ...
 @method parseTxHash
 @returns {Object} ok: boolean status, val: txHash string or hex error code 
*/
...
/**
 ...
 @method onGetPin
 @returns {Buffer} repr 32 character alpha-numeric pin string
*/ 
...
/**
 ...
 @returns {Buffer} JSON string repr. int value converted to string.
*/ 
...
/**
 ...
 @param {String} hex prefixed tx hash
 @returns {Number} Initial response is hex code callback. 0x00 on success or hex error code.
 @returns {Buffer} JSON object (or null string) repr. blockNumber: int OR null, nonce: int, gas: int
*/
...
/**
 Generates, saves and responds w/ a new session id. Required to access the submitTx endpoint.
 @method onGetNewSessionId
 @listens {Characteristic} Subscribe & write to: 9BBA5055-57CA-4F78-BA61-52F4154382CF
 @param {String} Current endpoint pin value, signed by caller account.
 @returns {Number} Initial response is hex code callback. 0x00 on success or hex error code.
 @returns {Buffer} JSON object (or null string) repr. sessionId: string, expires: int, account: address
*/
...
/**
 Responds w/ the transaction hash of the submitted tx from an atomic authAndSubmit 
 request. This is available once the submittedTx has been mined. Also returns authStatus
...
/**
 ...
 @param {String} Current endpoint pin value, signed by caller account.
 @returns {Number} Initial response is hex code callback. 0x00 on success or hex error code.
 @returns {Buffer} JSON object (or null string) repr. contract object incl. code, session data.
*/ 
...
/**
 ...
 @returns {Buffer} part of a JSONed contract object sitting in send queue. (see onGetContract)
 @returns {Buffer} JSON string EOF after last packet is sent.
*/ 
...
/**
 ...
 @param {String} Current endpoint pin value, signed by caller account.
 @returns {Number} Initial response is hex code callback. 0x00 on success or hex error code.
 @returns {Buffer} JSON string (or null string) repr. authTx transaction hash.
*/ 
...
/**
 ...
 Responds w/ auth transaction receipt. 
 @listens {Characteristic} Subscribe & write to: 8D8577B9-E2F0-4750-BB82-421750D9BF86
 @param {object} pin: Current endpoint pin value, signed by caller account.
                 tx: signed functionTx
 @returns {Number} Initial response is hex code callback. 0x00 on success or hex error code.
 @returns {Buffer} JSON string (or null string) repr. authTx transaction hash.
*/ 
const onAuthAndSubmitTx = function(data, offset, response, callback ){
    ...
    let parsedPin = parseSignedPin(data);
    let parsedTx = parseSignedTx(data, parsedPin.val );

    if (parsedPin.ok && parsedTx.ok){
        client = parsedPin.val;
        tx = parsedTx.val;

        callback(codes.RESULT_SUCCESS);
        eth.authTx( [pin, lastPin], client )
            .then( authTxHash => {
                eth.submitTxWhenAuthed(authTxHash, tx, client);
