...
// w/ a subset of the web3 data about the tx: whether it was mined or not, 
// the block number it was mined on. Responds with error code if tx not 
// found or input malformed.
    ...
    if (req.status){
        ...
        eth.getContract([pin, lastPin], req.val)
            ...
            .then( txHash => {
                ...
                setTimeout(()=>{ 
                    getContractCharacteristic.updateValueCallback(txHash);
                }, 50);   
            })
            .catch( error => { callback(error) });
    } else callback(req.val);      
