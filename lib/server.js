...
/**
 * Verified that JSON input has minimum formal properties of a sessionId and returns id as string.
 ...
 */
const canSubmitTx = function(data){
    ...
    return new Promise((resolve, reject) => {
        ...
        if(parsedId.ok){
            // Get session and see if client has outstanding auth event
            sessions.get(parsedId.val).then( session => {
                eth.db.get(session.account) 

                    // Case: YES: is Tx pending?  
                    .then( doc => {
                        (typeof doc.submittedTxHash != 'string')
                            ? parsedTx = { ok: false, val: code.TX_PENDING } 
                            : parsedTx = parseSignedTx(data, session.account);
                    })
                    // Case: NO
                    .catch( () => parsedTx = parseSignedTx(data, session.account))
                    
                    // Resolve/Reject
                    .finally( () => { 
                        (parsedTx.ok) 
                            ? resolve(parsedTx.val)  
                            : reject(parsedTx.val)
                    })
            })
            // Case: sessionId not found
            .catch( () => resolve({ok: false, val: codes.INVALID_SESSION_ID}))        
        
        // Case: sessionId didn't parse
        } else resolve(parsedId)  
    });
}
 ...
 * a pending or incomplete authAndSubmit tx exists in the contractDB for this caller.
...
const onSubmitTx = function(data, offset, response, callback){
    ...
    canSubmitTx(data)
        .then( tx => {
            ...
            callback(code.RESULT_SUCCESS);

            eth.submitTx(parsedTx.val)
                .then( txHash => {
