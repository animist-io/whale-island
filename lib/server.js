 ...
 * @param  {Object}  characteristic Bleno characteristic object 
 * @return {Boolean} True if uuid is not already being published, false otherwise
 */
const isUniqueUUID = function(uuid, characteristics){
 ...
 * @param {Object}  uuid    v4 characteristic uuid (for READ). 
 * @param {String}  message response to send to read requests at `uuid`.
 * @param {Number}  expires Date (Epoch ms) to stop publishing message.
 * @return {Promise} Result of DB gets/puts.
 */
const addPublication = function( args ){

    // Set up new characteristic
    let characteristic = new bleno.Characteristic({ uuid: args.uuid, properties: ['write']});
    characteristic.onWriteRequest = handlers.generatePublicationHandler(args, characteristic); 
       
    let list = [];
    let duration = args.expires - Date.now();
    let item = {characteristic: characteristic, expires: args.expires };
