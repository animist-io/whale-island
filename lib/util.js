...
/**
 ...
 @var {String} pin: 32 char alpha-numeric random string, signed to access certain endpoints.
...
const extractPinFromJSON = function (data) {
  try {
    ...
    let pinIsString = typeof parsed.pin === 'string'
    let pinIsObject = typeof parsed.pin === 'object'

    // Case: data has form { pin: object|string, tx: object }
    if (parsed.hasOwnProperty('pin') && (pinIsString || pinIsObject)) {
      return parsed.pin
    ...
    // Case: data neither ex: 0x4dea71bde50f23d347d6b21e18c50f02221c50ad
    } else {
      return null
    }
  } catch (e) {
...
const parseSignedPin = function (data) {
  ...
  if (parsed) {
    ...
    } else if (typeof parsed === 'string' && isHex(parsed)) {
      return { ok: true, val: parsed }
...
const parseTxHash = function (data) {
  try {
    const parsed = JSON.parse(data)
    return (typeof parsed === 'string' && isHex(parsed) && parsed.length === TX_HASH_LENGTH)
      ? { ok: true, val: parsed }
