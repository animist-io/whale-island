...
/**
 ...
 @param {Buffer} callback: init response is hex code . 0x00 on success or err. 
 @returns {Buffer} JSON formatted string BigNumber
 @returns {Buffer} JSON formatted string "null" on error. *** ????
*/
...
/**
 ...
 @property {Pin} Access
 @method onGetVerifiedTxHash
 @param {Buffer} data: JSON formatted pin value, signed by caller account.
 @param {Buffer} callback: init response is hex code . 0x00 on success or err. 
 @returns {Buffer} JSON formatted string txhash: "0x7d34e..023" 
 @returns {Buffer} JSON formatted string "null" on error.
*/ 
const onGetVerifiedTxHash = function(data, offset, response, callback){
    
    let client, out = {};   
    let pin = util.getPin();
    let self = defs.getVerifiedTxHashCharacteristic;
    let req = util.parseSignedPin(data);
...
module.exports = {
    ...
    onAuthTx : onAuthTx,
    onSendTx : onSendTx,
    onGetVerifiedTxHash : onGetVerifiedTxHash,
